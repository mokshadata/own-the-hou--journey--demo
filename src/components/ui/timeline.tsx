...
const Timeline: Component<TimelineProps> = (rawProps) => {
  ...
  return (
    <ul
      style={{
        "padding-left": `${others.bulletSize / 2}px`
      }}
      class={cn(local.class)}
    >
      <For each={others.items}>
        {(item, index) => (
          <TimelineItem
            title={item.title}
            description={item.description}
            bullet={item.bullet}
            isLast={index() === others.items.length - 1}
            isActive={others.activeItem === -1 ? false : others.activeItem >= index() + 1}
            isActiveBullet={others.activeItem === -1 ? false : others.activeItem >= index()}
            bulletSize={others.bulletSize}
            lineSize={others.lineSize}
